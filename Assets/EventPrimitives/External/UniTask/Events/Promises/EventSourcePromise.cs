...
namespace CatCode.EventPrimitives.Promises
{
    ...
    public sealed class EventSourcePromise<T> : IUniTaskSource
    {
        private readonly static ObjectPool<EventSourcePromise<T>> s_pool = new(() => new());

        static EventSourcePromise()
        {
            TaskPool.RegisterSizeGetter(typeof(EventSourcePromise<T>), () => s_pool.CountAll);
        }

        private readonly Action<T> _handler;
        private readonly Action _cancellationAction;

        private IReadOnlyEventSource<T> _source;
        ...
        private void Init(IReadOnlyEventSource<T> source, ICondition<T> predicate, CancellationToken cancellationToken)
        {
            _cancellationToken = cancellationToken;
            _source = source;
            _predicate = predicate;

            _source.Raised += _handler;
        ...
        public static IUniTaskSource Create(IReadOnlyEventSource<T> eventSource, ICondition<T> predicate, CancellationToken cancellationToken, out short token)
        {
            if (cancellationToken.IsCancellationRequested)
                return AutoResetUniTaskCompletionSource.CreateFromCanceled(cancellationToken, out token);

            var promise = s_pool.Get();
            promise.Init(eventSource, predicate, cancellationToken);
